...
-- For Wrapped Lines, move them down visually instead of by Line Number
map({ "n", "x" }, "j", "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true })
map({ "n", "x" }, "<Down>", "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true })
map({ "n", "x" }, "k", "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true })
map({ "n", "x" }, "<Up>", "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true })

-- DOUBT: Move to window using the <ctrl> hjkl keys, default is <C-w>hjkl not sure if it is useful
...
map("t", "<C-w>", "<cmd>close<cr>", { desc = "Hide Terminal" })
